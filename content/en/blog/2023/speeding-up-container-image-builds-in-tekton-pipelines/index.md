...
Example code and assets are available in this repo: [https://github.com/ggalloro/tekton-speed-builds](https://github.com/ggalloro/tekton-speed-builds)
...
* The [sample-app](https://github.com/ggalloro/tekton-speed-builds/tree/main/sample-app) folder contains source code, Maven project files, and a Dockerfile for an example Spring Boot Java app. Look at the [Dockerfile](https://github.com/ggalloro/tekton-speed-builds/blob/main/sample-app/Dockerfile): it’s multi-staged using the `maven` base image for packaging your code and the `eclipse-temurin:19-jdk-alpine` one to run your application:
...
* The [tekton-assets](https://github.com/ggalloro/tekton-speed-builds/tree/main/tekton-assets) folders contain resource manifests for the Tekton assets used in this article:
    * [pipeline.yaml](https://github.com/ggalloro/tekton-speed-builds/blob/main/tekton-assets/pipeline.yaml) defines a pipeline running 2 Tasks:
        * [git-clone.yaml](https://github.com/ggalloro/tekton-speed-builds/blob/main/tekton-assets/git-clone.yaml) clones the example repo to the source Tekton workspace 
        * [image-build.yaml](https://github.com/ggalloro/tekton-speed-builds/blob/main/tekton-assets/image-build.yaml) runs Kaniko to build a container image from the source code cloned in the workspace and push it to the target container registry
    * [source-pvc.yaml](https://github.com/ggalloro/tekton-speed-builds/blob/main/tekton-assets/source-pvc.yaml) is the manifest for the source-pvc persistent volume claim that will be used as persistent storage for the source Tekton workspace. The storage class is intentionally not defined to make it portable so it will use whatever is the default on your cluster, you can modify the manifest to use your preferred storage class.
* The [kaniko-basecache](https://github.com/ggalloro/tekton-speed-builds/tree/main/kaniko-basecache), [kaniko-cache](https://github.com/ggalloro/tekton-speed-builds/tree/main/kaniko-cache), and [m2cache](https://github.com/ggalloro/tekton-speed-builds/tree/main/m2cache) folders contain example resources useful to implement the various caching options.
...
Change the text in line 25 of the [index.html](https://github.com/ggalloro/tekton-speed-builds/blob/main/sample-app/src/main/resources/templates/index.html) file in [sample-app/src/main/resources/templates](https://github.com/ggalloro/tekton-speed-builds/tree/main/sample-app/src/main/resources/templates) to “Hello, Slow Builder!”
...
To do that you can apply the modified [image-build-cache.yaml](https://github.com/ggalloro/tekton-speed-builds/blob/main/kaniko-cache/image-build-cache.yaml) in the [kaniko-cache](https://github.com/ggalloro/tekton-speed-builds/tree/main/kaniko-cache) folder to update the image-build Task:
...
To do that you can apply the modified [image-build-basecache.yaml](https://github.com/ggalloro/tekton-speed-builds/blob/main/kaniko-basecache/image-build-basecache.yaml) in the [kaniko-basecache](https://github.com/ggalloro/tekton-speed-builds/tree/main/kaniko-basecachespeed-builds/tree/main/kaniko-cache) folder to update the image-build Task:
...
Let’s apply the modified [image-build-m2cache.yaml](https://github.com/ggalloro/tekton-speed-builds/blob/main/m2cache/image-build-m2cache.yaml) in the [m2cache](https://github.com/ggalloro/tekton-speed-builds/tree/main/m2cache) folder to update the image-build Task:
